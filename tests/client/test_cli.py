...
def patch_client(monkeypatch):
    ...
    monkeypatch.setattr(concord232.client.client.Client, 'list_zones', lambda self: [
        {'number': 1, 'name': 'Front', 'state': False, 'type': 'Door'},
        {'number': 2, 'name': 'Garage', 'state': True, 'type': 'Motion'},
    ])
    monkeypatch.setattr(concord232.client.client.Client, 'list_partitions', lambda self: [
        {'number': 1, 'zones': 2, 'arming_level': 'Off'},
        {'number': 2, 'zones': 1, 'arming_level': 'Stay'},
    ])
    monkeypatch.setattr(concord232.client.client.Client, 'send_keys', lambda self, keys, group, partition=None: True)
    monkeypatch.setattr(concord232.client.client.Client, 'get_version', lambda self: '1.2.3')
    monkeypatch.setattr(concord232.client.client.Client, 'disarm', lambda self, pin: True)
    monkeypatch.setattr(concord232.client.client.Client, 'arm', lambda self, level: True)
    yield
